...
| `scripts/` | `lib/routing.mjs` (recipient selection for the path), `migrate-input.mjs` (input/ → content/), `validate.mjs` (all merge checks), `check-links.mjs` (+ `linkcheck-exclude.yml`), `ingest-resources.mjs`, `file-link-issues.mjs`; `lib/` shared code (`content.mjs`, `geo.mjs`, `slug.mjs`, `i18n.mjs`, `codeowners.mjs`) |
...
| `site/` | Site source — developers only. `pages/[lang]/` (door `index`, `start/[outcome]` path, `directory`, `[section]/[id]` records, `about`, `resources`), `pages/api/[section].json.js`, `pages/version.json.js`, `layouts/Base.astro`, `lib/data.mjs` (build-time data, UI strings, cards, precomputed routing), `lib/measure.js` (completion events, sends nothing), `styles/global.css` |
...
| `.github/workflows/` | `check.yml` (validate, test, build, link-check changed files on PRs), `weekly.yml` (full link check + issues), `ingest.yml` (twice-daily Resource intake PR) |
...
- Build the site: `npm run build` (→ `dist/`, ~1 s, 430+ pages)
