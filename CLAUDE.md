...
| `scripts/` | `lib/routing.mjs` (recipient selection for the path), `migrate-input.mjs` (input/ → content/), `validate.mjs` (all merge checks), `check-links.mjs` (+ `linkcheck-exclude.yml`), `file-link-issues.mjs`; `lib/` shared code (`content.mjs`, `geo.mjs`, `slug.mjs`, `i18n.mjs`, `codeowners.mjs`) |
...
| `.github/workflows/` | `check.yml` (validate, test, build, link-check changed files on PRs), `weekly.yml` (full link check + issues) |
