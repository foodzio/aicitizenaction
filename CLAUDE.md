...
| `scripts/` | `migrate-input.mjs` (input/ → content/), `validate.mjs` (all merge checks), `check-links.mjs` (+ `linkcheck-exclude.yml`), `file-link-issues.mjs`; `lib/` shared code (`content.mjs`, `geo.mjs`, `slug.mjs`, `i18n.mjs`, `codeowners.mjs`) |
...
- Build / run locally: _TBD — Astro site not yet scaffolded (plan phase 3a)_
